...
// mail : gmanfredi.mail@gmail.comŮ
#include "rpnp.h"

using namespace std;
using namespace cv;

rpnp::rpnp(const cv::Mat& cameraMatrix, const cv::Mat& opoints, const cv::Mat& ipoints)
{
  ...
  number_of_correspondences_ = std::max(opoints.checkVector(3, CV_32F), opoints.checkVector(3, CV_64F));

  XX_.resize(3 * number_of_correspondences_);
  XXa_.resize(3 * number_of_correspondences_);
  XXc_.resize(3 * number_of_correspondences_);
  xx_.resize(2 * number_of_correspondences_);
  xxv_.resize(3 * number_of_correspondences_);
...
void rpnp::compute_pose(Mat &R, Mat &t)
{
  select_longest_edge ();
  ...
  compute_camera_poses_minima (rot, trans);
  Mat(3, 1, CV_64F, trans).copyTo(t);
  Mat(3, 3, CV_64F, rot[0]).copyTo(R);
}
////////////////////////////////////////////////////////////////////////////////
//  PRIVATE METHODS
////////////////////////////////////////////////////////////////////////////////
double rpnp::reprojection_error(const double R[3][3], const double t[3])
{
  double sum2 = 0.0;
...
double rpnp::compute_camera_poses_minima (double best_R[3][3], double best_t[3]) {
  ...
  for (int i=0; i<7; ++i) {
    ...
    if (t2s_[i] != 0) {
      ...
      */
      ...
      calcampose (&XXc_[0], &XX_[0], R, t); // DEBUG
  ...
  if (min_error > 5.0) {
    ostringstream msg;
    msg << "Warning: high reprojection error: " << min_error;
  }
  return min_error;
}
