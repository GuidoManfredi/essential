...
bool Matcher::homographyCheck (vector<KeyPoint> query_kpts,
                                vector<KeyPoint> object_kpts,
                                float reprojectionThreshold,
                                vector<DMatch>& matches,
                                Mat& homography) {
    if (matches.size() < _min_number_matches_allowed)
        return false;
    ...
    return (matches.size() > _min_mumber_matches_allowed);
}
