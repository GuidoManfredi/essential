...
bool segment (segment_plans_objects::PlantopSegmentation::Request  &req,
	...
	ROS_INFO("Query received !");
	if (current_cloud->empty()) {
		res.result = 1;
		ROS_ERROR ("segment_plans_objects : no current cloud");
		return false;
	}
	ROS_INFO ("Segmenting...");
	res.result = 4;
  if (segmenter.segment_plans (current_cloud) == 0)
		...
		res.result = 3;
	ROS_INFO("Segmentation finished with result %d.", res.result);
...
int main(int argc, char **argv) {
	/*
	if (argc != 3) {
		ROS_ERROR("Usage: segment_plans_objects in_pointcloud_topic in_rgb_topic");
		return 1;
	}
	*/
  ...
  ros::Subscriber sub_rgb = n.subscribe(argv[2], 10, save_rgb);
	// Output
  ros::ServiceServer srv_plans = n.advertiseService("/plans_objects_segmentation", segment);
 	ROS_INFO("Segmentation service ready.");
