...
class RecoOBB
{
 public:
	...
	std::vector< std::pair<std::string, float> > 
		recognize (float w, float h, float d, float confidence,
							 std::vector<std::string> candidates = std::vector<std::string>());
	void load_training_file (std::string stats_filepath);

 protected:
	std::vector< std::pair<std::string, float> >
		recognize (pcl::PointXYZ descriptor, float confidence,
							 std::vector<std::string> candidates = std::vector<std::string>());
	float mahalanobis_distance (Eigen::Matrix3f cov,
															Eigen::Vector3f norm_mean,
															pcl::PointXYZ pt);
