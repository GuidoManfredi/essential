...
class Visualizer:
    ...
    def draw(self):
        ...
        cv2.imshow ('Localisation !', self.img)
        cv2.waitKey(1)
...
def main(in_image_topic, in_pose_topic):
    rospy.init_node('opencv_display_pose', anonymous=True)
    rospy.Subscriber(in_pose_topic, LocaPose, pose_callback)
    rospy.Subscriber(in_image_topic, Image, image_callback)
...
help = '''Usage : display_pose in_image_topic in_pose_topic'''
